...
export const IncomeReport: React.FC<IncomeReportProps> = ({ transactions, onClick }) => {
  ...
  const reportData = useMemo(() => {
    ...
    const categoryTotals = income.reduce((acc, tx) => {
  ...
  return (
    <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 cursor-pointer hover:border-emerald-200 transition-all" onClick={onClick}>
      <div className="flex justify-between items-center mb-3">
        ...
        <h3 className="text-sm font-black text-emerald-700 text-center">توزيع الإيرادات</h3>
        <button onClick={(e) => { e.stopPropagation(); onClick(); }} className="text-emerald-700 text-[10px] font-semibold hover:underline">عرض الكل</button>
      </div>
